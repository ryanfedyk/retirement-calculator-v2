...
export const runSimulation = (
  ...
  for (let month = 0; month < monthsToSimulate; month++) {
    ...
    // The remaining mortgage balance IS part of the FI number, by design. To be
    // work-optional you need liquid assets that both fund your recurring expenses
    // (25× net need) AND handle the mortgage — you can't pay the loan with the
    // house itself, and the home is not counted among your spendable (investable)
    // assets. Any rental income the property throws off is already credited via
    // annualPassiveIncome above, which lowers the recurring need; the payoff term
    // then covers the debt. (Dropping this term makes the FI number understate a
    // large mortgage — e.g. a $1.6M target that silently assumes a $1.4M loan
    // vanishes.) Rent, being perpetual, is instead capitalized inside annualExpenses.
