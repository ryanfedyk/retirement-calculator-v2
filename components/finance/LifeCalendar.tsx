...
export default function LifeCalendar({ data, config }: Props) {
  ...
  data.forEach((pt, i) => {
    ...
    // Mortgage paid off: the month the payment drops from >0 to 0 (the real payoff).
    if (i > 0 && config.spending.housing_type !== "rent" && data[i-1].mortgagePayment > 0 && pt.mortgagePayment === 0) events.push("Mortgage Paid Off");
